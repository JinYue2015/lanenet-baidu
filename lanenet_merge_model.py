...
class LaneNet(cnn_basenet.CNNBaseModel):
    ...
    def compute_loss(self, input_tensor, binary_label, instance_label, name):
        ...
        with tf.variable_scope(name):
            ...
            print(" 标签维度 ",binary_label.shape.as_list()) #
            print(" 输出维度 ",decode_logits.shape.as_list())
            ...
            #print(" 损失维度 ",binary_segmenatation_loss)
...
if __name__ == '__main__':
    ...
    #img=tf.constant(1,shape=[1,256,512,3])
    img = np.ones(shape=[1,256,512,3])
    #binary_img=tf.constant(2,shape=[1,256,512,1])
    binary_img = np.ones(shape=[1,256,512,1])
    instance_img = np.ones(shape=[1,256,512,1])

    #instance_img=tf.constant(3,shape=[1,256,512,1])
    with tf.Session() as sess:
        init_op = tf.global_variables_initializer()
        sess.run(init_op)
        loss_ret = sess.run(ret,feed_dict={input_tensor:img,binary_label:binary_img,instance_label:instance_img})
        print(loss_ret['binary_seg_loss'])
