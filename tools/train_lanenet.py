...
def train_net(dataset_dir, weights_path=None, net_flag='vgg'):
    ...
    label_dataset_file = ops.join(dataset_dir,'label.txt')

    assert ops.exists(train_dataset_file)

    train_dataset = lanenet_data_processor.DataSet(train_dataset_file,label_dataset_file)
    val_dataset = lanenet_data_processor.DataSet(val_dataset_file,label_dataset_file)
    ...
    with tf.device('/gpu:3'):
        ...
        multi_seg_loss = compute_ret['multi_seg_loss']
        disc_loss = compute_ret['discriminative_loss']
        pix_embedding = compute_ret['instance_seg_logits']
        loss_var = compute_ret['l_var']
        loss_dist = compute_ret['l_dist']
        loss_reg = compute_ret['l_reg']
        
        multi_instance_logits = compute_ret['multi_seg_logits']
        multi_instance_logits=tf.nn.softmax(logits=multi_instance_logits)
        multi_instance_logits_out = tf.argmax(multi_instance_logits,axis=-1,output_type=tf.int64)
        miou,_=tf.metrics.mean_iou(instance_label_tensor,multi_instance_logits_out,num_classes=8)
    ...
    train_instance_seg_loss_scalar = tf.summary.scalar(name='train_instance_seg_loss', tensor=multi_seg_loss)
    val_instance_seg_loss_scalar = tf.summary.scalar(name='val_instance_seg_loss', tensor=multi_seg_loss)
    train_cluster_seg_loss_scalar = tf.summary.scalar(name='train_cluster_seg_loss', tensor=disc_loss)
    val_cluster_seg_loss_scalar = tf.summary.scalar(name='val_cluster_seg_loss', tensor=disc_loss)
    ...
    train_merge_summary_op = tf.summary.merge([train_accuracy_scalar, train_cost_scalar,
                                               ...
                                               train_instance_seg_loss_scalar,train_cluster_seg_loss_scalar])
    val_merge_summary_op = tf.summary.merge([val_accuracy_scalar, val_cost_scalar,
                                             val_binary_seg_loss_scalar, val_instance_seg_loss_scalar,val_cluster_seg_loss_scalar])
    ...
    with sess.as_default():
        ...
        if net_flag == 'vgg' and weights_path is None:
            ...
            pretrained_weights = np.load(vgg16_path,
        ...
        for epoch in range(train_epochs):
            ...
            _, c, train_accuracy, train_summary, binary_loss, cluster_loss, embedding, binary_seg_img ,instance_loss ,miou_x= \
                sess.run([optimizer, total_loss,
                          ...
                          disc_loss,
                          pix_embedding,
                          out_logits_out,multi_seg_loss,miou],
                         feed_dict={input_tensor: gt_imgs,
                                    binary_label_tensor: binary_gt_labels,
                                    instance_label_tensor: instance_gt_labels,
                                    phase: phase_train})
            #print("l_var",l_var," l_dist",l_dist," l_reg",l_reg)
            if math.isnan(c) or math.isnan(binary_loss) or math.isnan(cluster_loss):
                ...
                cv2.imwrite('nan_instance_label.png', instance_gt_labels[0]*30)
            ...
            if epoch % 100 == 0:
                ...
                cv2.imwrite('instance_label.png', instance_gt_labels[0]*30)
                cv2.imwrite('binary_seg_img.png', binary_seg_img[0] * 255)
                #cv2.imwrite('instance_seg_img.png',multi_instance_logits_out[0]*30)
                for i in range(4):
            ...
            if epoch % CFG.TRAIN.DISPLAY_STEP == 0:
                log.info('Epoch: {:d} total_loss= {:6f} binary_seg_loss= {:6f} instance_seg_loss= {:6f} cluster_seg_loss= {:6f} accuracy= {:6f}'
                         ' miou={:6f} mean_cost_time= {:5f}s '.
                         format(epoch + 1, c, binary_loss, instance_loss, cluster_loss, train_accuracy, miou_x,
                                np.mean(train_cost_time_mean)))
                train_cost_time_mean.clear()

            if epoch % CFG.TRAIN.TEST_DISPLAY_STEP == 0:
                log.info('Epoch_Val: {:d} total_loss= {:6f} binary_seg_loss= {:6f} '
                         'cluster_seg_loss= {:6f} accuracy= {:6f} '
                         'mean_cost_time= {:5f}s '.
...
if __name__ == '__main__':
    ...
    dataset_dir='E:/JY/dataset/baidu'
    weights_path='E:/JY/lanenet-lane-detection/tools/model/tusimple_lanenet/tusimple_lanenet_vgg_2018-12-07-10-18-20.ckpt-2000' # None # 
    net='vgg'
    train_net(dataset_dir,weights_path,net)
