...
import sys
sys.path.append('C:/Users/jy101/Desktop/lanenet-lane-detection')
from encoder_decoder_model import vgg_encoder_dilate       #
from encoder_decoder_model import fcn_decoder_dilate_fuse       #
...
class LaneNet(cnn_basenet.CNNBaseModel):
    ...
    def __init__(self, phase, net_flag='vgg'):
        ...
        if self._net_flag == 'vgg':
            self._encoder = vgg_encoder_dilate.VGG16Encoder(phase=phase)   #
        elif self._net_flag == 'dense':
            self._encoder = dense_encoder.DenseEncoder(l=20, growthrate=8,
                                                       with_bc=True,
                                                       phase=phase,
                                                       n=5)
        self._decoder = fcn_decoder_dilate_fuse.FCNDecoder(phase=phase)         #
        return
    ...
    def compute_loss(self, input_tensor, binary_label, instance_label, name):
        ...
        with tf.variable_scope(name):
            ...
            """
            binary_segmenatation_loss = focal_loss.focal_loss(decode_logits,binary_label,2)
            binary_segmenatation_loss = tf.reduce_mean(binary_segmenatation_loss)
            """
            # 计算多分类分割损失函数
            ...
            """
            multi_segmenatation_loss = focal_loss.focal_loss(decode_multilogits,instance_label,8)
            ...
            l2_reg_loss *= 0.001
            total_loss = 0.5 * binary_segmenatation_loss + l2_reg_loss +  0.5 * multi_segmenatation_loss  #+ 0.1 * disc_loss 
...
if __name__ == '__main__':
    ...
    binary_label = tf.placeholder(dtype=tf.int64, shape=[1, 512, 1024, 1], name='label')
    instance_label = tf.placeholder(dtype=tf.int64, shape=[1, 512, 1024, 1], name='label')
