...
class VGG16Encoder(cnn_basenet.CNNBaseModel):
    ...
    def _dilation_conv_stage(self, input_tensor, k_size, out_dims,rate, name,
                    stride=1, pad='SAME'):
        """
        空洞卷积层
        :param input_tensor:
        ...
        """
        with tf.variable_scope(name):
            conv = self.dilation_conv(input_tensor=input_tensor,k_size=k_size, out_dims=out_dims,
                                       rate = rate, use_bias=False,
    ...
    def encode(self, input_tensor, name):
        ...
        with tf.variable_scope(name):
            ...
            # conv stage 4_1
            conv_4_1 = self._dilation_conv_stage(input_tensor=pool3, k_size=3,
                                        out_dims=512,rate=2, name='conv4_1')

            # conv stage 4_2
            conv_4_2 = self._dilation_conv_stage(input_tensor=conv_4_1, k_size=3,
                                        out_dims=512,rate=2, name='conv4_2')

            # conv stage 4_3
            conv_4_3 = self._dilation_conv_stage(input_tensor=conv_4_2, k_size=3,
                                        out_dims=512,rate=2, name='conv4_3')

            # pool stage 4
            pool4 = conv_4_3
            #self.maxpooling(inputdata=conv_4_3, kernel_size=2,
                                    #stride=2, name='pool4')
            ret['pool4'] = dict()
            ...
            # conv stage 5_1
            conv_5_1 = self._dilation_conv_stage(input_tensor=pool4, k_size=3,
                                        out_dims=512,rate=4, name='conv5_1')

            # conv stage 5_2
            conv_5_2 = self._dilation_conv_stage(input_tensor=conv_5_1, k_size=3,
                                        out_dims=512,rate=4, name='conv5_2')

            # conv stage 5_3
            conv_5_3 = self._dilation_conv_stage(input_tensor=conv_5_2, k_size=3,
                                        out_dims=512,rate=4, name='conv5_3')

            # pool stage 5
            pool5 = conv_5_3
            #self.maxpooling(inputdata=conv_5_3, kernel_size=2,
                                    #stride=2, name='pool5')
            ret['pool5'] = dict()
