    ...
    def decode(self, input_tensor_dict, decode_layer_list, name):
        ...
        with tf.variable_scope(name):
            ...
            deconv_final = self.deconv2d(inputdata=score, out_channel=64, kernel_size=16,
                                         stride=8, use_bias=False, name='deconv_final')     ###作为特征映射的最后一层，也用来计算聚类loss

            score_final = self.conv2d(inputdata=deconv_final, out_channel=2,
                                      kernel_size=1, use_bias=False, name='score_final')    #############out_channel=2 作为二分类最终输出才有结果，就很奇怪

            score_x = self.conv2d(inputdata=deconv_final, out_channel=2,
                                              kernel_size=3,use_bias=False ,name='score_x')   ###
            concat_x = tf.concat([score_final,score_x],3,name='concat_x')                     ###将二分类特征与score_x层拼接
            multiclassify_final = self.conv2d(inputdata = concat_x,out_channel=8,
                                                kernel_size=3,use_bias=False,name='multiclassify_final') ###多分类最终输出
